...
{
    // Clear console screen using a safer method
    COORD topLeft = {0, 0};
    ...
    for (;;)
    {
        ...
        if (_kbhit())
        {
            ...
            {
                break; // Exit game loop if ESC is pressed
            }
        }
        ...
        // Check for wall collision (including upper bound)
        ...
        // Draw the snake (with bounds checking)
        for (int i = 0; i < bodylen; i++)
        {
            if (body[i].x >= 0 && body[i].x < W && body[i].y >= 0 && body[i].y < H)
            {
    ...
    // Display the player's score after the game ends
    system("cls");
    cout << "Player " << playerNum << " Game Over!" << endl;
...
int main()
{
    ...
    int score1 = 0, score2 = 0; // Initialize scores
